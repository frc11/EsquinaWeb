...
export default function ServicesSidebar() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(resolveActive, {
      rootMargin: `-${headerOffset + 2}px 0px 0px 0px`,
      threshold: 0,
    });
  ...
  const handleJump = useCallback(
    (event: React.MouseEvent<HTMLAnchorElement>, id: string) => {
      ...
      // Acá **no** se marca el ítem a mano. Se probó y sobra: el spy mide bien
      // el aterrizaje (verificado, desfase 0 px contra el header), y adelantarlo
      // solo agregaba un parpadeo —la flecha saltaba al destino y volvía atrás
      // en el cuadro siguiente, cuando el observer recalculaba sobre el scroll
      // todavía en el origen—. Sin eso, la flecha acompaña el desplazamiento y
      // llega al destino. Una sola fuente de verdad.
      const headerOffset = getHeaderOffset();
