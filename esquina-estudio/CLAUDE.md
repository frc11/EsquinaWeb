...
Última sincronización: **2026-09-01** (ronda **R2**, segunda tanda de devoluciones de las clientas: doce fases de producto sobre los dos PDF `Correcciones_Traducción` y `Correcciones_Mobile`, exportados a `docs/archivo/mockups/r2-*.jpg`. Lo que cambia el mapa mental del repo: **el banco de medición dejó de ser desechable** y vive en `tools/bench/` (§7b); **`ThreeLines` ya no existe** —la frase de la marca pasó a `CopyLines`, de largo variable, porque el castellano tiene dos líneas y el inglés tres (§6)—; **los sistemas de «aparecer» pasaron de cuatro a tres**, porque Contact y el intro de Services comparten `components/ui/entrance-fade.ts` (§6); **los párrafos de Team ya no llevan `
`** y con eso se fueron el `split`, el `whitespace-pre-line` y el `space-y-0` de la sección 03; **el cromo de escritorio tiene un escalón nuevo entre 1024 y 1151** que baja a 15 px, sin el cual `FUN GALLERY` se le monta a `CONTACTANOS` en castellano; y **`HOME_BLOCK_HEIGHT_MOBILE` pasó de 304 a 180** al reorganizarse el footer de mobile. Ver §6, §7b, §8 y la bitácora); antes, 2026-08-28 (sprint **M13**: el logo del preloader se achica en escritorio a **19 % del ancho del viewport**; mobile queda en el 37 % aprobado. El 19 % **sale de una cuenta, no de un gusto**: iguala la presencia relativa de mobile —el área del logo sobre el área de la pantalla—, y medido da 2,187 % en 390 × 844 contra 2,229 % en 1920 × 1080. Otra vez alcanzó con cambiar un valor de `--preloader-logo-share`; ver §6 y la bitácora); antes, 2026-08-28 (sprint **M12**: las proporciones del logo del preloader quedan al revés de como las dejó M11, que las había aplicado cruzadas); antes, 2026-08-28 (sprint **M11**: el tamaño del logo del preloader deja de depender del alto de la ventana y pasa a declararse, medido y no estimado; el video no se tocó); antes, 2026-08-27 (sprint **M10**: el orden y el reparto de Fun Gallery **de verdad** —lo que se veía en producción era el motor pre-M9, porque `main` estaba tres commits adelante de `origin/main` y M9 nunca se desplegó; además la última fila pasa a repartir su sobrante parejo a lo ancho, y la grilla de mobile de medias celdas a cuartos—; ver §6, §7b y la bitácora); antes, 2026-08-27 (sprint **M9**: el 500 del formulario —configuración de la cuenta de Resend, con el destinatario ya configurable y el error del proveedor registrado— y el orden y la distribución de Fun Gallery); antes, 2026-08-26 (sprint **M8**: el cierre de `/services` vuelve a su composición de escritorio y la cuadrícula queda acotada a mobile); antes, 2026-08-26 (sprint **M7**, limpieza y archivado del repo: la documentación histórica pasa a `docs/archivo/` —ver §9— y `docs/` queda en cinco entradas de consulta corriente; sin cambios de producto, los 32 altos idénticos); antes, 2026-08-26 (sprint **M6**, cierre de la ronda: el lienzo negro de `/studio`, la pantalla de éxito en teléfonos de 640, el campo `contentEs`, la limpieza del repo y **el archivo de trampas técnicas y límites del entorno de §7 y §7b**); antes, 2026-08-25 (sprint **M5**: la compuerta del preloader deja de escribir sobre `<html>`); antes, 2026-08-24 (sprint **M4**: el ícono único del menú, el estado activo del menú y los dos footers de mobile); antes, 2026-08-23 (sprint **M3**: el preloader nuevo y las quince correcciones); antes, 2026-08-23 (sprint **M2**: las catorce correcciones de mobile y el cierre de la adaptación); antes, 2026-08-22 (sprint **B4d**: el set de banderas dibujado a mano se retira y entran SVG reales vendorizados, en gris por defecto y a color en hover); antes, 2026-08-22 (sprint **B4c**: timing del toggle, limpieza de dependencias y corrección del set de banderas); 2026-08-22 (sprint **M1**, adaptación mobile); 2026-08-22 (microsprint B4b, refinamiento del toggle de idioma); **2026-08-21** (cierre de B4, idioma EN/ES, y con él el cierre de la ronda); antes de eso, 2026-08-20 (B3.4 + B3.4b, rediseño de `/services`) y 2026-08-15 (Bloque 1) sobre la auditoría completa `docs/reportes/2026-08-13-auditoria-completa.md` (HEAD `2565d01`). Las secciones no tocadas por B3.4 ni por B4 siguen reflejando esa auditoría. Antes de ejecutar cualquier sprint: leer `docs/plan-maestro.md` y la última entrada de `docs/bitacora.md`. Ante conflicto entre este archivo y el plan maestro, **manda el plan maestro**.
...
pedía 403 px, con el logo ocupando hasta 244 y el bloque de la derecha 217 — a
768 se montan, a 1024 entran. Y el `InfoRow` del footer pide **789 px** de ancho:
con el gutter de 64 entra recién a partir de 1024.

**Ese «con holgura» era falso, y R2 lo midió.** El menú en castellano pedía
**455,2 px** —no 403— y a 1024 dejaba **0,64 px** entre los glifos de `GALERÍA` y
los de `CONTACTANOS`: margen cero. Con `FUN GALLERY`, que es lo que pidieron las
clientas, se montaban 18,9 px. De ahí el **escalón de 1024 a 1151** que documenta
`Navbar.tsx`: en esa banda el cromo baja a 15 px con 24 de aire y la holgura pasa
a 42,2 px; de 1152 para arriba no cambia un píxel. **La lección: un menú centrado
en el viewport contra un bloque anclado al gutter no tiene ninguna defensa contra
que el copy crezca — hay que volver a medir cada vez que un rótulo cambia.**
...
- **Preloader (rehecho en M3/F1).** Es el **video del logo** que pasaron las clientas: `public/preloader-logo.mp4`, 173,0 KB, 36 cuadros a 12 fps, sin pista de audio y con el `moov` al frente. La cortina dura **3000 ms** —la del video— más el deslizamiento de salida de 1000 que ya existía; eran 2700 en total hasta M2. Sigue corriendo **solo la primera visita por pestaña** (`sessionStorage["esquina:preloaderShown"]`).
  ...
  **El mecanismo cambió, y es lo que hay que entender antes de tocarlo.** La cortina se sirve **en el HTML del servidor**, con estilos en línea, así que existe en el primer pintado; hasta M2 se montaba dentro de un `requestAnimationFrame` disparado desde un `useEffect`, o sea después de la hidratación, y por eso la página aparecía antes que ella. La regla de «una vez por pestaña» **no se resuelve en React**: la resuelve un **script bloqueante** en `layout.tsx` que lee `sessionStorage` y `prefers-reduced-motion` antes del primer pintado. Así el primer render del cliente es idéntico al del servidor y la hidratación no puede romperse. **Desde M5 ese script no escribe nada sobre `<html>`**: inyecta un `<style id="preloader-gate">` propio en `<head>` con la regla que corresponda, y `LoadingScreen` lo saca cuando la cortina empieza a irse. Hasta M4 marcaba `data-preloader` en `<html>`, y eso **sí** rompía la hidratación —en desarrollo—: el HTML servido no trae ese atributo y React avisa por los que encuentra y no rindió. El mecanismo entero, con la medición, está en `src/lib/preloader-gate.ts`.

  **El lienzo también se pinta de negro, y no es redundante con la cortina** (M3/F1b). El nodo de la cortina es lo segundo que hay dentro de `<body>`, pero el navegador puede pintar **antes** de haberlo parseado, y lo que pinta entonces es el `bg-off-white` del body: medido en un arranque lento, primer pintado a los 2198 ms con la pantalla en blanco y la cortina recién a los 2337, o sea ~140 ms de destello claro. Por eso el script bloqueante inyecta la regla que pinta `html` y `body` de negro cuando la cortina va a correr; la saca `LoadingScreen` cuando la cortina **empieza** a irse. Verificado estrangulando la red: el primer cuadro pintado da `YAVG = 0` a 40 y a 12 kB/s, contra los 243 de antes. **Matiz medido en M5:** ese `YAVG = 0` no es absoluto — a 12 kB/s aparece un cuadro off-white antes del negro en **1 de cada 10** arranques en frío, y **ya pasaba antes** (2 de 10 con el mecanismo del atributo, misma vara y mismas diez pasadas). Es una carrera entre el primer pintado y el script, no una regresión.

  **El tamaño del logo se declara, y sale de una medición (M11).** El pedido de las clientas llega como **ancho del logo sobre ancho del viewport**: **37 % debajo de 1024 y 19 % de ahí para arriba** (M11 los aplicó cruzados; **M12 los intercambió** y **M13 bajó el de escritorio de 27 a 19 %**, y las tres veces alcanzó con tocar un valor, porque el tamaño quedó parametrizado). **El 19 % sale de una cuenta:** iguala la *presencia relativa* que mobile ya tiene aprobada, o sea el área del logo sobre el área de la pantalla. Con el logo en 2,8473:1, un ancho del 37 % en un teléfono de 390 × 844 ocupa el 2,222 % del área; para dar lo mismo en un viewport de 16/9 el ancho tiene que ser 18,86 %, y 19 % da 2,254 %. Medido sobre el sitio servido: **2,187 % en 390 × 844 contra 2,229 % en 1920 × 1080**, un 1,9 % de diferencia. **Ojo con generalizar la paridad:** está definida contra un teléfono alto y un escritorio de 16/9 exacto, así que en un 1920 × 937 real el área sube a 2,570 % y en un 320 × 568 a 2,662 %. Lo que se fija —y lo que se verifica— es el **ancho**, que desde M11 no depende del alto de la ventana. No es un factor de escala, porque el logo no ocupa el cuadro entero: medida por umbral sobre los 36 cuadros, su caja más ancha —la del último, el logo terminado— es de **1119 × 393 px**, o sea el **58,281 % del ancho** del video. Hasta M10 el video iba `width: 100%; height: 100%` con `object-contain`, así que el ancho pintado era `min(ancho, alto × 16/9)` y **el porcentaje del logo dependía del alto de la ventana**: 58,2 % en un viewport de 16/9 pero 50,5 % en 1920 × 937, que es una ventana de escritorio real. Un objetivo escrito en «% del ancho del viewport» no se puede cumplir así. Ahora el ancho se **declara** en `vw` y la relación se fija en 16/9, de modo que el ancho pintado es el declarado en toda ventana (1920 × 1080 y 1920 × 937 dan el mismo 18,96 %). Las tres propiedades viven en `globals.css`: `--preloader-logo-ratio` (la constante **del asset**; si se reemplaza el video, se vuelve a medir), `--preloader-logo-share` (el objetivo del pedido, con la media query de 1024) y `--preloader-video-width`, que es la división. **Para cambiar el tamaño se toca `--preloader-logo-share` y nada más.** Va como variable de CSS y no como estado de React porque el corte de 1024 es una media query, y resolverla en el primer render del cliente es lo que rompió la hidratación en el precedente de arriba. Con la caja ya en 16/9 —la del video y la del póster, de 32 × 18— `object-contain` se vuelve una identidad y se conserva; el video se centra con flex y no con un `transform`, para no reintroducir los artefactos sub-pixel de §7.5. **Ojo:** el logo **no está centrado dentro del asset** —el centro de su trazo cae 45 px a la izquierda del centro del cuadro, 2,34 % del ancho—, así que en pantalla queda corrido ~0,8–1,6 % del ancho del viewport, según el rango. Es del archivo, no del código, y viene desde M3; el arreglo de raíz sería reencuadrar el video.
...
- **Mobile — `src/lib/mobile-layout.ts` y `src/lib/use-media-query.ts` (M1).** Los dos son únicos de su clase y están documentados en §2b: el primero lleva el gutter del cromo, el piso de área táctil y la nota sobre `sizes`; el segundo es **el** hook de media queries, del que ya cuelga `usePrefersReducedMotion`. Cualquier cosa nueva que necesite preguntar por el ancho desde JavaScript se cuelga de ahí; el layout, en cambio, se resuelve con variantes de Tailwind. **M2 le sumó dos medidas al primero**, las dos como clases enteras: `HOME_BLOCK_HEIGHT_MOBILE` (el alto del bloque del hero de `/` en mobile, que resta el alto del footer) y `HOME_FOOTER_CLEARANCE` (el hueco que ocupa ese footer y que `/contact/success` necesita porque ahí va superpuesto). **Desde R2/F11.3 el número de mobile es 180**: fue 236 en M2, 244 en M3, 304 en M4/F3 y ahora 180, porque el footer de mobile perdió su cuarta fila —el logo script pasó a la columna del medio— y con ella los 124 px que esa fila y su aire ocupaban. Los dos números son el alto real del `HomeFooter` en cada rango y **se verifican midiendo**, no se deducen. **Y la verificación no es el alto del documento**: `/` mide una pantalla exacta con el número bien o mal, porque el `mt-auto` del footer absorbe la diferencia. Lo que hay que medir es que el **borde inferior del bloque del hero toque el borde superior del footer** (holgura 0). Con el 304 viejo contra un footer de 180 quedaban 124 px de aire muerto, idénticos en los cuatro anchos de mobile, y ningún alto lo delataba.
...
- **Sistemas de «aparecer»: quedan 3, y bajaron de 4 en R2/F10.** Dos por scroll (inline de WorkGrid, `y:40` + stagger 0.7; `RevealOnScroll`, que solo usa Team) y **uno solo de entrada por opacidad**, `src/components/ui/entrance-fade.ts`, que comparten el intro de `/services` y el formulario de Contact. Antes eran dos entradas artesanales distintas: la de Services era solo opacidad y la de Contact animaba `clipPath` + `filter: blur` + `opacity` con una duración por propiedad. Las clientas pidieron que Contact «aparezca como la animación del texto de la sección de servicios», así que el patrón se extrajo a un módulo y Contact pasó a consumirlo; se fueron `CONTACT_EASE` y las tres variantes de recorte y desenfoque. **El Hero sigue aparte y a propósito**: anima `y` además de la opacidad, o sea que es otro gesto. El módulo define **cómo aparece cada elemento** y deja la cadencia del grupo a cada consumidor, porque el intro orquesta dos hijos y el formulario diez. `RevealLine` de `ServicesIntro` desapareció con el desmontaje de B3.4.
...
- **Banderas del selector de país — `CountryFlag.tsx` + `countryFlagCodes.ts` (B2, rehecho en B4d).** **Ya no son dibujos propios: son SVG reales, vendorizados.** Los archivos viven en `public/flags/`, nombrados por código ISO en minúsculas, y `COUNTRY_FLAG_CODES` lleva el nombre inglés del país a su código. **Fuente, versión y licencia están en `docs/archivo/banderas-set.md`** (`flag-icons` 7.5.0, MIT, verificada leyendo el `LICENSE` del paquete); el texto de la licencia viaja con las copias en `public/flags/LICENSE.txt`. **No es una dependencia y no va a serlo**: `package.json` no lo menciona, y actualizar el set es repetir el vendorizado a mano. **En reposo van en escala de grises y en hover a color**, que es el gesto que tenían — un filtro CSS sobre un solo archivo, no dos elementos apilados. Cinco reglas que hay que respetar al tocarlo:
...
- **Reparto en filas de la Fun Gallery — `getColumn` en `FunGallery.tsx` (M10). Es el único criterio y lo comparten los tres repartos.** Devuelve la columna de un objeto, **en celdas y posiblemente fraccionaria**, y la consumen tanto el motor de dispersión de escritorio como `buildGridLayout` para las grillas de teléfono y de tablet — así los tres no pueden discrepar. La regla: **se llena de arriba hacia abajo con filas completas y el sobrante va siempre en la última fila; nunca se equilibra el reparto entre filas.** La última fila reparte sus `k` objetos **parejo a lo ancho** —cada uno en el centro de su franja de ancho `columns / k`—, que con 4 columnas y 2 sobrantes los deja justo en los huecos de la fila de arriba (centros en 1/4 y 3/4). M9 corría la fila entera media celda por lugar sobrante: quedaba centrada pero **contigua**, con los objetos apiñados en el medio y el aire acumulado en los costados. Dos cosas que no se tocan sin volver a medir: **(1)** la grilla del DOM de mobile va en **cuartos de celda** (`grid-cols-8` en teléfono, `grid-cols-12` en tablet, `span_4` por objeto) y no en medias, porque el peor caso —dos objetos sueltos en la grilla de tres— cae en cuartos y con medias celdas el par salía asimétrico; **(2)** con cuartos, las columnas de los tres repartos caen todas en sub-columna entera, así que el `Math.round` del `colStart` **no redondea nada** — medido, asimetría 0,0000 % del ancho en los seis conteos de 8 a 13. En escritorio la simetría no es exacta y **eso es a propósito**: el motor suma su jitter de hasta 0,15 celda por eje, que es la identidad de la pantalla.
...
- Contratos frágiles conocidos (romperlos falla en silencio):
  - `TITLE_LINE_COUNT` (Hero) **no existe en el código**: este archivo lo arrastraba de la auditoría y se verificó en B4 que ya no está (cero ocurrencias en `src/`). **Y desde R2 tampoco hay un tipo que garantice el conteo**: `getHeroLines(locale)` devuelve `CopyLines`, de **largo variable**, porque las clientas pidieron el castellano en dos líneas y el inglés se queda en tres. La consecuencia está resuelta en `Hero.tsx` y conviene saberla antes de tocarla: al cambiar de idioma, el idioma más largo **monta un `<p>` nuevo** dentro de un padre que ya está en su variante final, y ese hijo se animaría solo. La `key` sigue siendo el índice —con el texto como clave se remontarían **las tres**, que es peor—; lo que evita la reanimación es que el hijo nuevo nazca con `initial={false}`. Verificado: opacidad 1 y `translateY` 0 en los 197 cuadros de la transición, en las dos direcciones. (El `TITLE_1_LINE_COUNT` de Services desapareció con B3.4, igual que el centinela `"Applications may include:"`, el `id="services-list"` y los dos `h-[200vh]` duplicados.)
...
- **Observar animaciones con la pestaña oculta.** Chrome no corre
  ...
  **La salida existe, está probada y desde R2 vive en el repo: `tools/bench/`.**
  Es Chrome `--headless=new` manejado por el DevTools Protocol sobre el
  `WebSocket` nativo de Node, sin dependencias. En headless la página se pinta de
  verdad, así que `rAF` corre, la carga diferida ocurre y las animaciones se
  pueden observar. **Usarlo, y no escribir uno nuevo.** Se construyó en M2 y se
  reconstruyó en M3, M4, M6 y R2 —cinco veces— antes de que alguien lo trackeara.

  - `cdp.mjs` levanta y conecta el navegador · `page.mjs` es una pestaña con
    viewport, idioma sembrado y espera de imágenes · `server.mjs` levanta y baja
    el servidor **por PID** · `matrix.mjs` es la matriz de no-regresión y sus dos
    sondas · `baseline.mjs` produce la vara · `diff.mjs` la compara ·
    `probe.mjs` corre una sonda arbitraria sobre ruta × ancho × idioma, con
    estrangulamiento de red y script previo al documento · `shot.mjs` captura.
  - **Tres cosas que las cinco reconstrucciones volvieron a equivocar**, y que
    `page.mjs` ya resuelve: el idioma hay que sembrarlo con
    `Page.addScriptToEvaluateOnNewDocument` (si se hace después de navegar llega
    tarde); sin `sessionStorage["esquina:preloaderShown"]` la cortina tapa tres
    segundos de cada medición; y hay que esperar a que **todas** las imágenes
    estén `complete` o el alto del documento varía ±1 px entre corridas.
  - Las varas quedan en `docs/archivo/mediciones/`, una por ronda.
...
### Contra qué se verifica — la lección de M10

- **Medir la rama no es medir el sitio.** M9 verificó su arreglo de Fun Gallery
  sobre `build` + `start` local, donde funcionaba, y lo reportó como si fuera lo
  que ve el visitante. No lo era: `main` estaba **tres commits adelante de
  `origin/main`** y producción seguía sirviendo el motor viejo. **Antes de
  declarar arreglado algo que el usuario ve en producción, correr
  `git status -sb` y mirar el `ahead/behind`.** Si la rama está adelante, lo
  verificado es una promesa, no un hecho, y hay que decirlo con esas palabras.
- **Un dato del dataset no se cita de memoria ni de la ficha.** Las clientas
  editan Sanity en vivo. §5 afirmaba 9 imágenes con `order` del 1 al 9 y había
  8, del 1 al 8. La API pública contesta sin token
  (`https://<projectId>.api.sanity.io/v2024-01-01/data/query/production?query=…`);
  son tres líneas de `fetch` y elimina toda la clase de error. Ojo con
  `apicdn`, que puede venir atrasada — para verificar, `api`.
- **La sonda tiene que apuntar a lo que dice medir.** La primera versión del
  banco de M10 buscaba «el ancestro común de las imágenes» y enganchó el
  `<body>`: midió el logo del header y el del footer y reportó ocho objetos
  centrados al 50 %. Un contenedor se localiza por una marca **suya**
  —`[style*="--d-aspect"]` en este caso—, no por inferencia sobre el árbol.
- **`next build` reusa su caché de fetch entre builds consecutivos.** Con
  `revalidate: 60`, dos builds separados por menos de un minuto sirven el mismo
  resultado de query aunque los datos hayan cambiado; en M10 eso hizo que la
  prueba de 10 objetos rindiera 9 y la de 12 rindiera 11, en silencio y sin un
  solo aviso. Para variar los datos entre builds hay que limpiar
  `.next/cache/fetch-cache` —generada e ignorada por git— antes de cada uno.
- **Framer no ve un `dispatchEvent` sintético.** Ni el tap del despliegue ni el
  `whileHover` reaccionan a eventos construidos a mano; hace falta
  `Input.dispatchMouseEvent` del DevTools Protocol, que Chrome emite como
  confiables. Y el hover pide **salir y volver a entrar**: un solo `mouseMoved`
  a la posición del objeto no genera el `pointerenter` si el puntero ya estaba
  dentro de su caja.
...
- **Dónde vive cada cosa en `docs/` (reordenado en M7).** De consulta corriente: `plan-maestro.md`, `pendientes.md`, `bitacora.md` —el registro vivo del método— y `reportes/` —la auditoría del 13 de agosto y el postmortem de la ronda, la base de hechos—. **Material histórico: `docs/archivo/`**, con su propio `README.md`; ahí están los **mockups** (las 15 páginas de `Final.pdf` más los cinco tramos de `/services`, que son la fuente de verdad del diseño y están medidos, no descritos), las **instrucciones** de la ronda, `banderas-set.md` y `sanity-piezas-es.md`. **No se borra documentación: se archiva.** Y si algo de ahí se vuelve a mover, primero el grep de la ruta vieja: hay comentarios de `src/` que citan mockups por ruta.
- **Cuando un pedido sale de mirar el teléfono, la instrucción tiene que decir «solo mobile». Sin esa palabra, el pedido es ambiguo y se ejecuta mal.** Lo dejó escrito R2 y no es teoría: las clientas anotan sobre capturas de su propio teléfono, así que «achicar el logo», «alinear a la izquierda» o «agrandar los elementos» **describen lo que ellas ven, no una regla del sitio**. Aplicado sin acotar, cada uno de esos tres habría movido el escritorio, que está aprobado y medido. La regla operativa: **antes de tocar nada que venga de un mockup de teléfono, escribir el rango en la propia instrucción** (`max-lg:`, `<1024`), y si la instrucción que llegó no lo dice, **pararse y preguntar en vez de deducirlo**. El corolario es el de siempre —el escritorio ≥1024 no cambia salvo pedido explícito— pero el error no está en el corolario sino en el momento en que se escribe la tarea.
- **Y el reverso, que R2 también pagó: una constante que resume una medición envejece sin avisar.** `HOME_BLOCK_HEIGHT_MOBILE` decía 304 —el alto del footer de mobile— y el footer pasó a medir 180 en la misma ronda. **Ningún alto de documento lo delata**, porque `/` mide una pantalla exacta pase lo que pase: el `mt-auto` del footer se come la diferencia. Se detecta midiendo **el borde inferior del bloque contra el borde superior del footer**, que tiene que dar 0. Es la tercera aparición de esta clase de defecto (punto 13 de M2, trampa 2 de §7.1).
...
- **El banco de medición ya no se reconstruye: vive en `tools/bench/` y se trackea** (R2). Van cinco reconstrucciones y las cinco volvieron a equivocarse en las mismas tres cosas —sembrar el idioma tarde, no saltear la cortina del preloader y medir con imágenes sin decodificar—. Ver §7b.
...
Después de la ronda —M7 fue limpieza y archivado— siguieron tres sprints de
producto.

**M10 — El orden y el reparto de Fun Gallery, de verdad** (2026-08-27, un
objetivo) cierra lo que M9/F2 dio por cerrado. El sitio en producción seguía
mostrando el reparto con huecos y el orden sorteado, y la causa raíz **no era
el código**: `main` estaba **tres commits adelante de `origin/main`**, así que
producción corría `ff34d7b` —el motor con `shuffle`— y M9 nunca llegó a
desplegarse. La verificación de M9 midió el código de la rama, que estaba bien,
y lo reportó como si fuera el sitio. Reproducido: el motor pre-M9 con los ocho
`_id` reales da el orden de lectura 4, 3, 8, 7, 6, 5, 1, 2 —exactamente lo que
se veía— y con diez ítems reparte 3+4+3 dejando dos de sus doce celdas vacías.
Con eso entendido, M10 corrigió además el reparto real: la última fila
**reparte su sobrante parejo a lo ancho** en vez de apiñarlo contiguo en el
medio (ver §6, `getColumn`), y la grilla de mobile pasó de medias celdas a
**cuartos**, que es la resolución que ese reparto necesita. **La regla operativa
que deja: una verificación que no se corre contra lo que ve el visitante no es
una verificación.** Detalle y mediciones en `docs/bitacora.md`.

Antes, **M9 — Formulario en producción y orden de Fun Gallery** (2026-08-27,
tres fases) resolvió dos problemas independientes. El formulario de contacto
devolvía **500 en producción** desde que el destino pasó a la casilla de las
clientas: el motivo, reproducido contra la API real, es que **Resend no acepta
destinatarios arbitrarios mientras el remitente esté en su dominio de prueba y
no haya un dominio propio verificado** (`403 validation_error`). Es
configuración de la cuenta, no código; lo que sí cambió en el repo es que el
destinatario salga de `CONTACT_TO_EMAIL` —no hardcodeado— y que el error del
proveedor **quede registrado del lado del servidor**. Y la Fun Gallery pasó a
**respetar el número del Studio y a repartirse sin huecos**: las celdas ya no se
sortean, las filas son de cuatro y el resto de la última fila se centra. **Ese
arreglo era correcto pero no llegó a producción, y su verificación no midió el
sitio: lo corrige M10** (ver arriba). Detalle y mediciones en la entrada de
`docs/bitacora.md`.

Antes, **M8 — Latest Projects vuelve a su composición de escritorio**
(2026-08-26, una fase): M3/F6 había mandado las portadas del cierre de
`/services` a cuadrícula y los dos links debajo **en todos los anchos**, y las
dos cosas valían solo para mobile. M8 devolvió el escritorio a la fila con
`flex-1` y los links a la fila del encabezado, dejando **mobile sin mover un
píxel** (12 de 12 geometrías idénticas en los cinco anchos y los dos idiomas).
