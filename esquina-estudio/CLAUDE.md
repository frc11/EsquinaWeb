...
- **Banderas del selector de país — `MonochromeCountryFlag.tsx` + `countryFlagColors.ts` (B2, corregido en B4c).** **No son emojis ni un paquete: son SVG propios**, y por eso se ven igual en Mac que en Windows. La forma: **44 patrones geométricos** repartidos entre los 196 países, con line-art monocromo en reposo y color en hover — el color es contenido, nunca cromo (§8). Cuatro reglas que hay que respetar al tocarlo:
  1. **Un país sin patrón no falla: cae en un comodín.** Eso es lo que rompió el set hasta B4c, y la lista de países crece: cada alta nueva tiene que entrar en un conjunto explícito. Hay un chequeo posible y barato — que **ningún país llegue al `return` final del `default`**.
  2. **La paleta tiene que traer tantas ranuras como pide el patrón**, y el orden de las ranuras es parte del contrato: está tabulado en el encabezado de `countryFlagColors.ts`. Una paleta corta no rompe nada visible: el país se queda en line-art aunque esté en hover, que es como estaban ocho de ellos.
  3. **Antes de dibujar un patrón nuevo, buscar si la estructura ya existe.** El orden es reasignar → crear el patrón una vez y compartirlo → dibujar aparte, y solo si es único e irreductible. B4c revivió `canton-stripes`, que estaba declarado y sin consumidores, en vez de escribir uno paralelo.
  4. **La caja del SVG (`h-[15px] w-[24px]`) y el `viewBox` no se tocan.** El fit de Contact está medido al píxel y el truncado que se arregló en B2.7 vuelve si la fila cambia de ancho.
  Lo que es **por país y no por patrón** —hoy, si el triángulo del asta lleva emblema— viaja como parámetro de `drawPattern`, no duplicando el enum.
